...
from src.shared.exceptions.domain import BusinessRuleError, DomainException, ForbiddenError
...
async def list_tasks_by_status(
    ...
    tasks, total = await task_service.get_tasks_by_status(status, skip=skip, limit=limit)
    return PaginatedResponse(items=tasks, total=total, skip=skip, limit=limit)
